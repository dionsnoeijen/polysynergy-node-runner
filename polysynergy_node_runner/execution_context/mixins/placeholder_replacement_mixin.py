    ...
    def _apply_placeholder_replacements(self):
        for attr_name in getattr(type(self), '__annotations__', {}):
            ...
            if isinstance(val, str):
                ...
                replaced = replace_placeholders(data=replaced, values=self.__dict__, state=self.context.state)
                setattr(self, attr_name, replaced)
            elif isinstance(val, (dict, list)):
                replaced = replace_placeholders(data=val, values=self.__dict__, state=self.context.state)
                setattr(self, attr_name, replaced)
